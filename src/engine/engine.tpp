...
Engine<ForceModel, ParticleType>::Engine(const std::chrono::duration<double> dt) :
...
template<typename ForceModel, typename ParticleType>
Engine<ForceModel, ParticleType>::Engine(const std::string& restart_file_name) :
        collision_detector_(particles_, surfaces_) {
    ...
    ParameterMap engine_parameters;
    ...
    object_id_counter_ = engine_parameters.get_parameter<std::size_t>("number_of_objects");
    collision_id_counter_= engine_parameters.get_parameter<std::size_t>("number_of_collision_objects");
    ...
    if (!keyword_data["*periodicbc"].empty() ){
...
template<typename ForceModel, typename ParticleType>
void Engine<ForceModel, ParticleType>::setup()
{
...
template<typename ForceModel, typename ParticleType>
void Engine<ForceModel, ParticleType>::setup(double bounding_box_stretch) {
...
void Engine<ForceModel, ParticleType>::run(Condition& condition)
{
    ...
    while (condition()) {
        ...
        if (time_to_log <= increment_) {
            ...
            auto [pid, speed] = max_particle_velocity();
            std::cout << "Fastest particle is " <<  pid << " with a speed of " <<  speed << std::endl;
        }
    }
...
template<typename ForceModel, typename ParticleType>
FineCylinder<ForceModel, ParticleType>* Engine<ForceModel, ParticleType>::create_fine_cylinder(const double radius, const double height,
    const double fine_mass, const double stone_density, const std::size_t numpoints)
{
    fine_cylinder_ = new FineCylinder<ForceModel, ParticleType>(radius, height, fine_mass, stone_density, numpoints);
    return fine_cylinder_;
}
...
template<typename ForceModel, typename ParticleType>
typename Engine<ForceModel, ParticleType>::OutputPointerType
Engine<ForceModel, ParticleType>::create_output(std::string directory, std::chrono::duration<double> interval){
...
void DEM::Engine<ForceModel, ParticleType>::
        remove_output(const OutputPointerType& output_to_remove)
{
...
SurfaceT DEM::Engine<ForceModel, ParticleType>::get_surface(const std::string& surface_name) const {
    ...
    auto surface = dynamic_cast<SurfaceT>(*it);
    if (surface) {
        return surface;
    }
    std::ostringstream error_ss;
...
template<typename ForceModel, typename ParticleType>
void Engine<ForceModel, ParticleType>::do_step()
{
    ...
    run_output();
    if (fine_cylinder_) {
        fine_cylinder_->update(increment_, gravity_);
    }
}
